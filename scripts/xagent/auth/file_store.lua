...
local function protect()
    if fs.is_windows then
        ...
        assert(os.execute('if not exist "' .. p .. '" mkdir "' .. p .. '" >nul 2>nul')
            ...
            'Cannot protect credential directory')
    else
        assert(os.execute('mkdir -p ' .. shell_quote(directory) .. ' && chmod 700 ' .. shell_quote(directory)),
            'Cannot protect credential directory')
    end
end
-- new(name) -> { load, save } for ~/.xagent/auth/<name>.json
function M.new(name)
    local S = {}
    ...
    function S.load()
        ...
        assert(type(parsed) == 'table', 'Invalid ' .. name .. ' credential file')
        return parsed
    end
    function S.save(value)
        protect()
        if not value then
